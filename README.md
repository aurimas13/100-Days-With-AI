...
[![Day](https://img.shields.io/badge/Day-51%20of%20100-1F6FEB?style=for-the-badge&labelColor=0D1117)](#-progress)
...
`2026-07-12` ──────────── **Day 51 of 100** ────────────► `2026-10-19`
...
| 51 | 2026-08-31 | "Agent Tools" - Cursor Docs | Medium | The reference page for what Cursor's agent can actually do, and more useful for what the list implies than for the list itself: the agent is described as three parts, "Instructions", "Tools" and a "Model", and the tools are file and folder search, file reading with image formats passed to a vision model, edits that it will "Suggest edits to files and apply them automatically", web search, rule retrieval, terminal commands where it will "Execute terminal commands and monitor output", browser control to "take screenshots, test applications, and verify visual changes", image generation, and the ability to ask clarifying questions mid-task; there is no error-handling section anywhere on the page, and that absence is the finding, because the recovery loop is not a feature but a property that falls out of three ordinary ones - running a command and watching what it prints, the flat promise that "There is no limit on the number of tool calls Agent can make during a task", and automatic checkpoints taken "before making significant changes, capturing the state of all modified files" and "stored locally and separate from Git", which makes a wrong turn cheap to undo; the practical consequence is that an agent can only close the loop over failures its own tools can observe, which is why Replit's FAQ can say its agent "often detects" a crash and fixes it in real time and in the same breath tell the human to read the deployment log first and to report a bug as "When input is X, I expected Y but got Z" | [cursor.com](https://cursor.com/docs/agent/tools) |
...
### Day 51 - "Agent Tools" (Cursor Docs)

<img src="assets/cards/day-051.png" width="420" alt="Day 51 card">

- **There is no error-handling section on the page, and that is the finding.** I went to this page looking for how a coding agent recovers from a failed command, a red test or a broken edit, and there is no such heading. No retry policy, no backoff, no description of what the agent does when a build fails. For a page that exists to enumerate capabilities, the omission is not sloppiness. The recovery loop is not one of the tools, it is what you get when three of them are put in the same room.
- **The three ordinary tools that assemble into a loop.** First, the terminal entry does not say "run commands", it says the agent will "Execute terminal commands and monitor output" - the monitoring is half the sentence and all of the point. Second, the page states flatly that "There is no limit on the number of tool calls Agent can make during a task", so nothing structural stops it from running, reading the failure and running again. Third, checkpoints: the system "automatically creates them before making significant changes, capturing the state of all modified files", and they are "stored locally and separate from Git". Observe, iterate without a cap, and roll back cheaply. That is an error loop, described without ever using the word.
- **An agent can only close the loop over failures its tools can observe.** This is the line I keep coming back to. A stack trace printed by a command the agent itself ran is inside the loop. A visual regression, a wrong number on a dashboard, a function that does exactly what was asked and not what was meant - none of those are, unless something puts them there. Read that way, the browser tool stops looking like a convenience and starts looking like an aperture: "Control a browser to take screenshots, test applications, and verify visual changes" is a way of turning a class of invisible failure into observable output. Vision on `.png`, `.jpg`, `.gif`, `.webp` and `.svg` is the same move for anything already on disk.
- **Where the agent cannot see, the human is the sensor - and vendors say so openly.** Replit's FAQ makes the split unusually explicit. On one side, the agent's own loop: "If the code it wrote crashes or throws an error, the Agent often detects it and attempts to fix the issue in real time." On the other, the human's job when it does not: read the deployment or console logs first, and hand back a report shaped as "When input is X, I expected Y but got Z". That template is doing real work. It supplies the input, the expectation and the observation - the three things a failing test would have carried automatically. It is a manual reconstruction of a signal the agent could not generate for itself.
- **The upshot for how I set up a task, which is the practical half.** If the agent fixes what it can see fail, then the highest-leverage thing I control is not the prompt, it is how much of the failure surface is wired into something that prints. A test command, a type check, a linter, a smoke script that exits non-zero: each one converts a silent defect into terminal output the loop already knows how to consume. The corollary is uncomfortable and worth saying plainly - on a task with no executable check, an agent is not iterating towards correctness at all, it is producing plausible text and I am the only sensor in the circuit.
- **What this entry does not accept without saying so.** Three limits. First, this is vendor documentation, not an evaluation: it describes intended behaviour and reports no measurement, and "there is no limit on the number of tool calls" is a product statement about a fast-moving tool, not a guarantee I have tested. Second, Replit's "often detects" is doing quiet work in that sentence, and a vendor FAQ is the least adversarial possible account of its own failure rate. Third, the supporting error-API link I had queued for today, Windsurf's, now redirects to Devin's documentation and turns out to describe analytics query errors rather than anything an agent feeds back to itself, so it is cited here only as the reason it went unused.

**Why it matters:** the phrase "error handling" invites you to look for a component, and in an agent there is no component to find. What there is instead is an observation surface. Whether a failure reaches the model is decided upstream of the model entirely, by which tools exist and what they print, and that is an engineering decision made before a single token is generated. It also reframes a complaint I have made myself. When an agent loops uselessly on a problem, the instinct is to blame its reasoning; more often the loop is blind, iterating against output that never contained the failure in the first place. Yesterday's four buckets were about what enters the context window on purpose. This is about what enters it by consequence, and the second kind is the only kind that carries a correction.

**What I learned/tried:** the correction to my own habit is small and specific. I have been treating the tool list of an agent as a capability question, as in what can it reach, and I now think it is at least as much an observability question, as in what can it notice going wrong. Those give different answers about what to add next. On my own pipelines the thing I will change is the order of setup: before writing the instructions, wire one command that fails loudly and non-zero when the output is wrong, and give the agent that command. I also want to test the uncomfortable half of the claim above rather than assert it. The channel's own daily run is a fair subject - it has a preflight script that exits non-zero on a malformed package, and it would be honest to check how many of the defects that have actually shipped this month were things that script could see, and how many only ever showed up in a human reading the file.
...
<div align="center">
<br>
<sub><b>Day 51 of 100.</b> Next entry tomorrow, ~7:00 EEST.</sub>
</div>
